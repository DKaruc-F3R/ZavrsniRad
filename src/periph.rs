...
pub fn peripheral_input_signal(pin: u8, per_sig: u8, pull: Pull) {
    ...
    unsafe { write_volatile(in_sel_reg, (1 << 31) | (pin as u32 & 0x3F)) };

    // Ažuriraj OUT_SEL registar povezan s pinom
    let out_sel_reg = (OUT_SEL_REG_BASE + 4 * pin as usize) as *mut u32;
    let mut val = unsafe { read_volatile(out_sel_reg) };
    val |= 1 << 10; // Ažuriraj OUT_SEL kontrolne bitove
    unsafe { write_volatile(out_sel_reg, val) };

    // Onemogući lokalni GPIO driver za taj pin
    if pin < 32 {
        unsafe { write_volatile(gpio_mux::GPIO_ENABLE_W1TC, 1 << pin) };
    } else {
        unsafe { write_volatile(gpio_mux::GPIO_ENABLE1_W1TC, 1 << (pin - 32)) };
    }

    // Konfiguriraj IO_MUX, odaberi GPIO funkciju i omogući ulazni buffer
    let io_mux_reg = io_mux::io_mux_reg(pin);
    let mut m = unsafe { read_volatile(io_mux_reg) };
    ...
    unsafe { write_volatile(io_mux_reg, m) };
}
...
pub fn peripheral_output_signal(pin: u8, per_sig: u8, open_drain: bool, drive_strenght: u8) {
    ...
    let mut v = unsafe { read_volatile(out_sel_reg) };

    // Postavi novi identifikator signala (pogledati tablici u TRM za id. od signala)
    v = (v & !0x1FF) | (per_sig as u32 & 0x1FF);
    // Postavi OUT_SEL
    v |= 1 << 10; //mozda na 11
    unsafe { write_volatile(out_sel_reg, v) };

    // Omogući lokalni GPIO driver
    if pin < 32 {
        unsafe { write_volatile(gpio_mux::GPIO_ENABLE_W1TS, 1 << pin) };
    } else {
        unsafe { write_volatile(gpio_mux::GPIO_ENABLE1_W1TS, 1 << (pin - 32)) };
    }

    // Konfiguriraj open-drain
    let pin_reg = (gpio_mux::GPIO_BASE + 0x88 + (pin as usize) * 4) as *mut u32;
    let mut drain = unsafe { read_volatile(pin_reg) };
    if open_drain {
        drain |= 1 << 2;
    } else {
        drain &= !(1 << 2);
    }
    unsafe { write_volatile(pin_reg, drain) };
